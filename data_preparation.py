...
# --- 加载数据 ---
file_path = r"C:\Users\Clanc\Desktop\DMT\A2\dmt-2025-2nd-assignment\training_set_VU_DM.csv"
...
# Click Rate & Book Rate Analyse
print("\n--- User Behavior (Click & Booking) ---")
click_rate = df['click_bool'].mean()
book_rate = df['booking_bool'].mean()
print(f"Click Rate: {click_rate:.2%}, Book Rate: {book_rate:.2%}")

sns.barplot(x=['Click', 'Booking'], y=[click_rate, book_rate])
plt.title('Click and Booking Rate')
plt.ylabel('Rate')
plt.show()


# 酒店评分越高，点击率是否越高？价格越低，预订率是否越高？
print("\n--- CTR/Booking Rate vs Rating ---")
df['rating_bin'] = pd.cut(df['prop_review_score'], bins=[0, 2, 3, 4, 5], include_lowest=True)
click_rate_by_rating = df.groupby('rating_bin')['click_bool'].mean()
book_rate_by_rating = df.groupby('rating_bin')['booking_bool'].mean()

plt.figure(figsize=(10, 5))
click_rate_by_rating.plot(label='Click Rate', marker='o')
book_rate_by_rating.plot(label='Booking Rate', marker='s')
plt.title('Click/Booking Rate by Review Score')
plt.legend()
plt.show()
...
sns.histplot(search_sizes, bins=50)
...
# 筛选出价格合理的数据进行分析
filtered_prices = df[df['price_usd'] <= price_cap]['price_usd']

plt.figure(figsize=(10, 5))
sns.histplot(filtered_prices, bins=50, kde=True)
plt.title('Hotel Price Distribution (Capped at 99th percentile)')
plt.xlabel('Price (USD)')
plt.tight_layout()
plt.show()

print("Filtered price statistics:")
print(filtered_prices.describe())



# 酒店星级与点击/预订行为关系
print("\n--- CTR and Booking Rate vs Hotel Star Rating ---")
star_click_rate = df.groupby('prop_starrating')['click_bool'].mean()
star_book_rate = df.groupby('prop_starrating')['booking_bool'].mean()

df_star = pd.DataFrame({'Click Rate': star_click_rate, 'Booking Rate': star_book_rate})
df_star.plot(kind='bar', figsize=(10, 5))
plt.title("Click & Booking Rate by Hotel Star Rating")
plt.ylabel("Rate")
...
print("\n--- Hotel Location Score Distribution ---")
plt.figure(figsize=(10, 5))
sns.histplot(df['prop_location_score1'].dropna(), bins=50, kde=True)
plt.title('Distribution of prop_location_score1')
plt.xlabel('Location Score 1')
plt.ylabel('Frequency')
plt.tight_layout()
plt.show()

# 将位置得分分箱并计算各段点击率
print("\n--- CTR vs Location Score (Binned) ---")
df['loc_score_bin'] = pd.cut(df['prop_location_score1'], bins=5)
loc_click_rate = df.groupby('loc_score_bin')['click_bool'].mean()

# 可视化
loc_click_rate.plot(kind='bar', figsize=(10, 5), title='Click Rate by Location Score Bin')
plt.ylabel('Click Rate')
plt.xlabel('Location Score Range')
plt.tight_layout()
plt.show()
